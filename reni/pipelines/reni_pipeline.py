...
class RENIPipeline(VanillaPipeline):
    ...
    def reset_train_latents_to_zero(self) -> None:
        """Reset train latents on the underlying model."""
        self.model.reset_train_latents_to_zero()

    def _eval_image_to_ray_bundle(self, eval_image_output):
        """Normalize eval image output across RENI and vanilla datamanagers."""
        if len(eval_image_output) == 3:
            image_idx, ray_bundle, batch = eval_image_output
            return int(image_idx), ray_bundle, batch

        camera, batch = eval_image_output
        image_idx = int(torch.as_tensor(batch["image_idx"]).reshape(-1)[0])
        ray_bundle = camera.generate_rays(camera_indices=0)
        ray_bundle.set_camera_indices(image_idx)
        return image_idx, ray_bundle, batch

    @staticmethod
    def _flatten_eval_ray_bundle(ray_bundle):
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: Optional[bool] = None):
        ...
        try:
            ...
            if not strict:
                ...
                for name, param in self.model.named_parameters():
                    if name in model_state:
                        if param.shape != model_state[name].shape:
                            print(f"Shape mismatch for {name}: model {param.shape} vs checkpoint {model_state[name].shape}. Removing from state_dict.")
                ...
                if "field.eval_mu" in model_state and model_state["field.eval_mu"].shape != self.model.field.eval_mu.shape:
                    model_state["field.eval_mu"] = torch.zeros_like(self.model.field.eval_mu)
                if "field.eval_logvar" in model_state and model_state["field.eval_logvar"].shape != self.model.field.eval_logvar.shape:
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        self._optimise_evaluation_latents(step)
        image_idx, ray_bundle, batch = self._eval_image_to_ray_bundle(self.datamanager.next_eval_image(step))
        # ray_bundle is already generated by datamanager, just reshape
        ray_bundle = self._flatten_eval_ray_bundle(ray_bundle)
        ray_bundle.camera_indices = torch.ones_like(ray_bundle.camera_indices) * image_idx
        outputs = self.model(ray_bundle)
        # image_idx is already returned from next_eval_image
    ...
    def get_average_eval_image_metrics(self, step: Optional[int] = None, optimise_latents: bool = True):
        ...
        """
        self.eval()
        # Only optimize eval latents if requested (skip for models with pre-loaded latents like old RENI)
        if optimise_latents:
            self._optimise_evaluation_latents(step)
        metrics_dict_list = []
        ...
        with Progress(
            ...
            for idx, eval_image_output in enumerate(self.datamanager.fixed_indices_eval_dataloader):
                image_idx, ray_bundle, batch = self._eval_image_to_ray_bundle(eval_image_output)
                # ray_bundle is already generated by datamanager, just reshape
                ray_bundle = self._flatten_eval_ray_bundle(ray_bundle)
                ray_bundle.camera_indices = torch.ones_like(ray_bundle.camera_indices) * image_idx
