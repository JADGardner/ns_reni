...
class EnvironmentMapField(SphericalField):
    ...
    def __init__(
        ...
        if self.config.trainable:
            self.train_envmaps = nn.Parameter(torch.randn(num_train_data, 3, self.resolution[0], self.resolution[1]), requires_grad=True)
            self.eval_envmaps = nn.Parameter(torch.randn(num_eval_data, 3, self.resolution[0], self.resolution[1]), requires_grad=True)
        else:
            ...
            if self.apply_padding:
              ...
              image = torch.nn.functional.pad(
                  image,
                  (0, 0, 1, 1),  # padding of 1 on either side of width dimension
                  mode='replicate'
              )
