...
def get_graph_feature_cross(x, k=20, dims=3, idx=None):
    batch_size = x.size(0)
    num_points = x.size(3)
    x = x.view(batch_size, -1, num_points)
    if idx is None:
    ...
    _, num_dims, _ = x.size()
    num_dims = num_dims // dims

    x = x.transpose(2, 1).contiguous()   # (batch_size, num_points, num_dims)  -> (batch_size*num_points, num_dims) #   batch_size * num_points * k + range(0, batch_size*num_points)
    feature = x.view(batch_size*num_points, -1)[idx, :]
    feature = feature.view(batch_size, num_points, k, num_dims, dims) 
    x = x.view(batch_size, num_points, 1, num_dims, dims).repeat(1, 1, k, 1, 1)
...
class VNBatchNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x: point features of shape [B, N_feat, 3, N_samples, ...]
        '''
        norm = torch.sqrt((x*x).sum(2)) # Element-wise norm
...
class VNStdFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dim == 3:
            ...
            x_std = torch.einsum('bijmno,bjkmno->bikmno', x, z0)
