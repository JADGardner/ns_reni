#!/usr/bin/env bash
# .apptainer/apptainer.sh — Unified Apptainer wrapper for ns_reni container.
#
# Commands:
...
MODEL_STORAGE_PATH="${MODEL_STORAGE_PATH:-${PROJECT_ROOT}/checkpoints}"
...
do_install() {
    ...
    if [[ ${#PASSTHROUGH_ARGS[@]} -gt 0 ]]; then
        ...
        # Default: register ns_reni code into /overlay-packages
        echo "=== Registering ns_reni in /overlay-packages ==="
        apptainer exec "${args[@]}" "${SIF_FILE}" bash -c "
            set -euo pipefail
            mkdir -p /overlay-packages

            echo '[1/1] ns_reni'
            echo '  Registering ns_reni ...'
            pip install --target /overlay-packages -e /workspace/code --no-deps --no-cache-dir --no-build-isolation
        "
    fi
...
build_run_args() {
    ...
    # Overlay: read-only + writable-tmpfs so multiple sessions can run concurrently.
    # Only `install` needs a writable overlay.
    if [[ -f "${OVERLAY_FILE}" ]]; then
