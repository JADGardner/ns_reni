...
_GROUND_TYPES = {
    "grass":   dict(alb=(0.070, 0.160, 0.045), tex=0.55, gloss=0.02,
                    var=(0.10, 0.16, 0.05), green=0.45, wt=0.26),
    "grass_lush": dict(alb=(0.045, 0.200, 0.035), tex=0.50, gloss=0.03,
                       var=(0.06, 0.18, 0.04), green=0.58, wt=0.17),
    "grass_dry": dict(alb=(0.150, 0.155, 0.070), tex=0.60, gloss=0.02,
                      var=(0.14, 0.12, 0.05), green=0.35, wt=0.11),
    "dirt":    dict(alb=(0.17, 0.115, 0.075), tex=0.50, gloss=0.03,
                    var=(0.10, 0.07, 0.05), green=0.0, wt=0.12),
    "snow":    dict(alb=(0.72, 0.740, 0.820), tex=0.14, gloss=0.60,
                    var=(0.05, 0.05, 0.07), green=0.0, wt=0.08),
    "asphalt": dict(alb=(0.075, 0.075, 0.085), tex=0.28, gloss=0.28,
                    var=(0.03, 0.03, 0.04), green=0.0, wt=0.11),
    "sand":    dict(alb=(0.30, 0.255, 0.180), tex=0.45, gloss=0.05,
                    var=(0.10, 0.08, 0.05), green=0.0, wt=0.15),
}
...
def render_sky(
    ...
    # Sun lobe (added in linear; sharpness ~ inverse angular width). The disc
    # radiance is attenuated by the airmass-dependent transmittance, so low
    # suns are dim and red, high suns bright and near-white.
    lobe = torch.exp(sun_sharpness * (cos_gamma - 1.0))
    sun_col = sun_transmittance(el, turbidity)
    sky = sky + sun_intensity * Yz * lobe[..., None] * sun_col
    ...
    # its own chroma instead of washing out blue against the sky fill.
    amb = mean_sky * (1.0 - gt["green"]) + mean_sky.mean() * gt["green"]

    # Irradiance: sky fill + warm direct sun with an azimuthal hotspot, then a
    # warm re-tint toward the sun at low elevations. Both terms keep a floor
    # independent of view direction so the foreground stays visibly lit toward
    # the nadir instead of crushing to black under the tone-map gauge.
    E = (1.6 * amb[None, None]
         + sun_up * sun_col[None, None] * (1.10 + 0.80 * bounce[..., None]))
    E = E * (1.0 + 0.60 * low * bounce[..., None] * (warm[None, None] - 1.0))
    ground = alb[None, None] * E * tex_mult[..., None]
    ground = ground + (torch.tensor(gt["var"])
                       * torch.clamp(tex, min=0.0)[..., None]) * 0.15 * amb
    haze = torch.exp(-torch.clamp(h_terr - elev, min=0.0) / 0.25)[..., None]
    ground = ground + 0.10 * haze * mean_sky[None, None]
...
def sample_params(rng: np.random.Generator) -> dict:
    return {
        "sun_elevation_deg": float(rng.uniform(2.0, 85.0)),
    ...
    if args.emit_pairs:
        ...
        for i in range(args.emit_pairs):
            ...
            d_b = _el_az_to_dir(alt["sun_elevation_deg"],
                                alt["sun_azimuth_deg"])
            mid = _dir_to_el_az(_slerp(d_a, d_b, 0.5))
        ...
        (args.output / "sun_labels.json").write_text(json.dumps(labels, indent=1))
        print(f"[saved] {args.emit_pairs} triplets + sun_labels.json -> {args.output}")
