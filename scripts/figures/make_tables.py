...
DIM_GROUPS = {
    27: [("SH", "sh", "2nd"), ("SG", "sg", 30),
         ("RENI", "reni_old", 9), ("RENI++", "reni_pp", 9),
         ("RENI++ (thesis)", "vnjoint_ortho_2cyc_testfit", 9)],
    108: [("SH", "sh", "5th"), ("SG", "sg", 108),
          ("RENI", "reni_old", 36), ("RENI++", "reni_pp", 36),
          ("RENI++ (thesis)", "vnjoint_ortho_2cyc_testfit", 36)],
    147: [("SH", "sh", "6th"), ("SG", "sg", 150),
          ("RENI", "reni_old", 49), ("RENI++", "reni_pp", 49),
          ("RENI++ (thesis)", "vnjoint_ortho_2cyc_testfit", 49)],
    300: [("SH", "sh", "9th"), ("SG", "sg", 300),
...
# (fixed gauge: scored raw, no scale alignment).
TWO_BRACKET_BANK_FAMILIES = {"vnjoint_ortho_2cyc_testfit"}


def evaluate_two_bracket_bank(family: str, key, device: str):
    import glob
    ...
    _, _, model = load_model(MODEL_DIRS[family][key], device=device)
    assert getattr(model, "two_bracket", False), f"{family}/{key}: not two-bracket"
    H, W = 64, 128
    bank = model.field.eval_mu.detach()
    files = sorted(glob.glob(str(REPO_ROOT / "data/RENI_HDR/test/*.exr")))
    assert bank.shape[0] == len(files), \
        f"{family}/{key}: bank {bank.shape[0]} vs {len(files)} test EXRs"
    rb = equirect_ray_bundle(device, idx=0, height=H)
    sums = {m: 0.0 for m in METRICS}
    for i, f in enumerate(files):
        z = bank[i:i + 1].to(device)
        with torch.no_grad():
            samples = model.create_ray_samples(
                rb.origins, rb.directions, rb.camera_indices)
            ...
            pred_lin = model._to_linear_hdr(out.reshape(-1, 6)).reshape(H, W, 3)
        pred = linear_to_sRGB(pred_lin, use_quantile=True)
        img = pyexr.read(f).astype("float32")[..., :3]
        img[img <= 0] = img[img > 0].min()
        img = F.interpolate(torch.tensor(img).permute(2, 0, 1)[None],
                            size=(H, W), mode="bilinear")[0].permute(1, 2, 0)
...
def evaluate(family: str, key, device: str):
    ...
    if family in TWO_BRACKET_BANK_FAMILIES:
        return evaluate_two_bracket_bank(family, key, device)
